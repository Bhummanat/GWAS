...
This script is used to filter for SNPs that passes the FDR test and produce KM plot for those SNPs
...
Change input_dir as required (for the .rds files so far in the pipeline)
...
FDR, change p.adjust as necessary
```{r Global FDR Correction for Significant SNPs}
# Apply FDR correction for basic SNPs and save significant ones
...
# Apply FDR correction for generalized SNPs and save significant ones
...
```{r Load allele data in manageable batches and filter for significant SNPs}
...
This exports the data in per patient basis (required for roc_test.R)
```{r Process Patient Data for ROC Test}
...
Setup directory for KM plots
```{r Ensure KM_Plots directory exists}
dir.create(paste0(input_dir, 'KM_Plots'), recursive = TRUE, showWarnings = FALSE)
```

Wasn't sure if this batch size is still required after fixing vcf_subsetting.sh issues (reducing size by 3x)
```{r batch size to manage memory}
batch_size <- 2  
...
map(position_batches, ~ {
  walk(.x, ~ {
    ...
    ggsave(filename = str_c('KM_Plot_', unique(.x$position), '.png'), 
           plot = plot, 
           path = paste0(input_dir, 'KM_Plots'),
           dpi = 100)
